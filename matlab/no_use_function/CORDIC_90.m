function [angle_out, x_out, y_out, flag_out] = ...
         CORDIC_90(x, y, angle_in, flag_in, options)
% mode 0:vectoring mode(clockwise), mode 1:rotation mode(anticlockwise)
    arguments(Input)
        ...
        options.stage (1,1) {mustBePositive} = 20
        options.mode (1,1) = 0 
...
init_angle = truncation(angle_in,options.input_angle_length);
    ...
    if(options.mode==0)
        if(x<0)
            ...
            mapping_flag = 1;
            if(y<0)
                x_data(1) = abs(truncation(y,options.input_length));
                y_data(1) = truncation(x,options.input_length);
            else 
                x_data(1) = truncation(y,options.input_length);
                y_data(1) = abs(truncation(x,options.input_length));
            end
        else
            mapping_flag = 0;
    ...
    elseif (options.mode==1)
        if(flag_in==1 && init_angle>=0)
            x_data(1) = -1*truncation(y,options.input_length);
            y_data(1) = truncation(x,options.input_length);
            angle_data(1) = init_angle;
        elseif (flag_in==1 && init_angle<=0)
            x_data(1) = truncation(y,options.input_length);
            y_data(1) = -1*truncation(x,options.input_length);
            angle_data(1) = init_angle;
        else
            x_data(1) = truncation(x,options.input_length);
            y_data(1) = truncation(y,options.input_length);
